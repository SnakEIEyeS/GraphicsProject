...
void main()
{
	...
	vec4 VertexNormal_CameraSpace = transpose(inverse(u_Camera)) * u_Object * o_VertexNormalToFS_ModelSpace;
	...
	float cosTheta = clamp(dot(LightDirection_CameraSpace, VertexNormal_CameraSpace), 0.f, 1.f);
	...
	vec4 FinalAmbientColor = FinalDiffuseColor * u_AmbientConstant;
	...
	o_ColorFS = FinalDiffuseColor + FinalAmbientColor + FinalSpecularColor;
}