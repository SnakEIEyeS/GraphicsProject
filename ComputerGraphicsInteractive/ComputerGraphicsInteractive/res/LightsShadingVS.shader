...
void main()
{
	...
	vec4 VertexPosition_CameraSpace = u_Camera * vec4(i_Pos, 1.f);
	...
	vec4 ViewDirection_CameraSpace = -normalize(u_Camera * vec4(i_Pos, 1.f));
	vec4 VertexNormal = transpose(inverse(u_Camera)) * vec4(i_Normal, 0.f);
