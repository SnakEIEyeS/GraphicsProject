...
void main()
{
	vec4 VertexPosition_CameraSpace = u_Camera * u_Object * o_VertexPosition;
	...
	vec3 mappedNormal = texture(u_NormalMapSampler, vec2(o_VertexTexture)).rgb;
	mappedNormal = normalize(mappedNormal * 2.f - 1.f);
	mappedNormal = normalize(o_TBNMat * mappedNormal);
